...
//
// Compatibility shim for GetWindowSubclass
//
// Some Windows builds do not export GetWindowSubclass from comctl32.dll, which
// causes a loader failure at startup if we link to it directly. We track
// subclass data locally and only call the exported Set/Remove APIs.
//
...
unsafe fn subclass_collection() -> &'static Mutex<HashMap<SubclassId, DWORD_PTR>> {
    if SUBCLASS_COLLECTION.is_none() {
        SUBCLASS_COLLECTION = Some(Mutex::new(HashMap::new()));
    }
    SUBCLASS_COLLECTION.as_ref().expect("subclass collection initialized")
}
...
unsafe fn SetWindowSubclass(hwnd: HWND, proc: SUBCLASSPROC, uid: UINT_PTR, data: DWORD_PTR) -> BOOL {
    ...
    let result = SetWindowSubclass(hwnd, proc, uid, data);
    if result != 0 {
        let id = (hwnd as usize, mem::transmute(proc), uid);
...
unsafe fn RemoveWindowSubclass(hwnd: HWND, proc: SUBCLASSPROC, uid: UINT_PTR) -> BOOL {
    ...
    let result = RemoveWindowSubclass(hwnd, proc, uid);
    if result != 0 {
        let id = (hwnd as usize, mem::transmute(proc), uid);
        let mut collection = subclass_collection().lock().unwrap();
